...
#   - Tools scripts (Install / Uninstall / Scan) from virtual/emulators/sources — self-contained
#     except they source modules/scripts/heroic_common.inc (uploaded from standalone/heroic/scripts).
#   - Copies standalone heroic scripts to /storage/.config/heroic-launchers/ (fallback when /usr/bin is
#     not populated yet). Rom stubs try /usr/bin first, then that directory.
#   - MODULES_ROOT_EXTRA (e.g. gamelist.xml) + images/heroic.svg + scripts/heroic_common.inc
...
SRC_HEROIC_COMMON="${ROCKNIX_ROOT}/packages/emulators/standalone/heroic/scripts/heroic_common.sh"
SRC_HEROIC_SCRIPTS="${ROCKNIX_ROOT}/packages/emulators/standalone/heroic/scripts"
...
[[ -f "${SRC_HEROIC_COMMON}" ]] || { echo "Missing: ${SRC_HEROIC_COMMON}" >&2; exit 1; }
...
_ssh "${ROCKNIX_HOST}" "${symlink_cleanup}rm -f '${ROCKNIX_REMOTE_MODULES}/${HEROIC_LEGACY_REMOVED_ON_DEVICE}' '${ROCKNIX_REMOTE_MODULES}/Start Heroic Authenticate.sh' '${ROCKNIX_REMOTE_MODULES}/Start Heroic Play.sh' '${ROCKNIX_REMOTE_MODULES}/Start Heroic Play Gamescope.sh' '${ROCKNIX_REMOTE_MODULES}/scripts/heroic-keyboard-toggle.sh' '${ROCKNIX_REMOTE_MODULES}/scripts/heroic_common.sh'"
...
_scp "${SRC_HEROIC_COMMON}" "${ROCKNIX_HOST}:${ROCKNIX_REMOTE_MODULES}/scripts/heroic_common.inc"

chmod_cmd="chmod 0755"
for name in "${HEROIC_MODULE_SCRIPTS[@]}"; do
  chmod_cmd+=" '${ROCKNIX_REMOTE_MODULES}/${name}'"
done
chmod_cmd+=" ; chmod 0644 '${ROCKNIX_REMOTE_MODULES}/scripts/heroic_common.inc'"
_ssh "${ROCKNIX_HOST}" "${chmod_cmd}"

# Only gameplay helpers (not Tools install/uninstall/scan duplicates).
HEROIC_LAUNCHER_FILES=(
  heroic_common.sh
  heroic-keyboard-toggle.sh
  start_heroic_authenticate.sh
  start_heroic_play.sh
  start_heroic_play_gamescope.sh
)
heroic_launcher_local=()
for bn in "${HEROIC_LAUNCHER_FILES[@]}"; do
  f="${SRC_HEROIC_SCRIPTS}/${bn}"
  [[ -f "${f}" ]] || { echo "Missing: ${f}" >&2; exit 1; }
  heroic_launcher_local+=("${f}")
done
_ssh "${ROCKNIX_HOST}" "mkdir -p '${ROCKNIX_REMOTE_HEROIC_LAUNCHERS}'"
_scp "${heroic_launcher_local[@]}" "${ROCKNIX_HOST}:${ROCKNIX_REMOTE_HEROIC_LAUNCHERS}/"
_ssh "${ROCKNIX_HOST}" "for f in '${ROCKNIX_REMOTE_HEROIC_LAUNCHERS}'/*.sh; do [ \"\$(basename \"\$f\")\" = heroic_common.sh ] && chmod 0644 \"\$f\" || chmod 0755 \"\$f\"; done"
...
echo "Heroic launchers uploaded to ${ROCKNIX_HOST}:${ROCKNIX_REMOTE_HEROIC_LAUNCHERS}/"
...
ls_cmd+=" '${ROCKNIX_REMOTE_MODULES}/images/heroic.svg' '${ROCKNIX_REMOTE_MODULES}/scripts/heroic_common.inc'"
_ssh "${ROCKNIX_HOST}" "${ls_cmd}"
_ssh "${ROCKNIX_HOST}" "ls -la '${ROCKNIX_REMOTE_HEROIC_LAUNCHERS}'"
